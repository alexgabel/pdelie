...
def test_the_warning_names_both_exact_fixes(field) -> None:
    """The two concrete fixes a caller can apply.

    The *version* the warning promises is no longer checked here: that moved to
    ``tests/test_forward_promises.py``, which scans every forward promise in
    ``src/`` rather than this one API's wording. Checking it in two places meant
    a second promise added elsewhere was covered by neither.
    """
