...
def test_v0_13_release_gate_metadata_docs_and_ci_are_aligned() -> None:
    ...
    readiness = _repo_text("docs/releases/V0_24_RELEASE_READINESS.md")
    ...
    assert release_gate_jobs == ["v0_24-release-gate"]
    assert "python -m pytest tests/test_v0_24_release_gate.py" in workflow
    assert "v0_13-release-gate" not in workflow

    assert "## 0.13.0" in changelog
    assert "V0.24" in readme
    assert "compute_periodic_window_coverage" in readme
    assert "diagnose_uniform_translation_consistency" in readme
    assert "package version: `0.24.0`" in readiness
    assert "git tag: `v0.24.0`" in readiness
    assert "Do not publish to TestPyPI or PyPI for `v0.24.0`" in readiness
    assert "including `v0.24.0`" in publishing
    assert "V0.24 is complete" in plan
