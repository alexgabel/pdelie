...
def run_reaction_diffusion_vertical_slice_example() -> dict[str, object]:
    field = generate_reaction_diffusion_1d_field_batch(batch_size=_BATCH_SIZE, seed=_GENERATOR_SEED)
    ...
    return summarize_vertical_slice(
        ...
        verification=report,
        extra_metrics={
            "example_name": "reaction_diffusion_vertical_slice",
            "equation": "reaction_diffusion_fisher_kpp",
...
def main() -> None:
    print(json.dumps(run_reaction_diffusion_vertical_slice_example(), indent=2))
