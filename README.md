...
```bash
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.37.0"
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.37.0#egg=pdelie[pdebench]"       # optional PDEBench cookbook extra (h5py)
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.37.0#egg=pdelie[downstream]"    # optional PySINDy bridge extra
```

The repository is public and MIT-licensed, so `pip install git+https://…` works without SSH keys, personal access tokens, or a package-index account. Pin a specific tag (e.g. `@v0.37.0`) for reproducibility. TestPyPI and PyPI publication remain deferred to `v1.0` — v0.36f built and hardened the publish path (SHA-pinned actions, build-to-publish hash verification) but deliberately did not exercise it; see [`docs/releases/PUBLISHING.md`](docs/releases/PUBLISHING.md).
