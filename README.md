...
The current release is `v0.38.0rc1` / **V0.38 release candidate** — feature-complete for v0.38 and **not yet a release candidate**: Gate F (the cross-platform replay) has now **closed** — three runner cells, 229 gate rows each, worst cross-platform scaled difference `4.168e-10` against a derived `1e-8` bound and bitwise-identical across a CPython patch bump (run `31328966332`, [Appendix D](docs/design/v0_38_platform_replay.md)) — and the rc1 cut follows. It keeps a two-release-old promise by requiring an explicit integer `seed` in `inspect_pysindy_weak_pde_library` (**breaking**; see [`V0_38_SEED_MIGRATION.md`](docs/releases/V0_38_SEED_MIGRATION.md)), makes parameter-action targets a declared field so a rescale can no longer silently touch every numeric parameter, and resolves the residual operator once from declaration and data provenance — blocking when they disagree. Irregular-grid work (v0.38a–d) is **not** in this alpha; pin it by exact commit for reconnaissance, not for confirmatory evidence. The last stable release is `v0.37.1` / **V0.37**: declarative problem-action contracts with five independent relation axes (`pdelie.actions`), an exact-shift executor over six runtime paths, a residual commutation report separating what a benchmark expected from what measurement observed, a five-case admissibility benchmark behind a two-stage freeze (`pdelie.benchmarks`), and a downstream crash test whose blocking branches are measured to return before discovery runs (`pdelie.downstream`). All diagnostic-only and submodule-only. **`v0.37.1` corrects benchmark case C-5**, which as released declared a parameter rescale and measured a state rescale — see [`V0_37_C5_ERRATUM.md`](docs/releases/V0_37_C5_ERRATUM.md). `discovery_task_result` keeps its 22-key schema, frozen since v0.30.1. `v0.36.0` / **V0.36** shipped the paper-critical migration audit — 100 stage comparisons across five PDEs, zero unexplained regressions, verified on macOS and Linux — plus content-addressed artifact identity, the audit layer, observation and differentiation specifications, and budget-aware design comparison. `v0.35.0` / **V0.35** shipped design-matrix diagnostics (mutual coherence, leverage scores, the irrepresentability constant, and a support-restricted eigenvalue), three deterministic row-selection methods, and a private catalogue of known Lie point symmetries. `v0.34.0` / **V0.34** shipped variable-coefficient residual evaluators for Heat / Burgers / advection-diffusion (dispatching on the equation form the v0.33d generators record), reference-relative admissibility scoring and background-treatment classification, and opt-in column normalization for the weak-form design matrix with a reproducibility seed. `v0.33.0` / **V0.33** shipped nonperiodic interior-symmetry and mask-validity support: the generator chain (`fit_translation_generator` → `polynomial_translation_svd` → `verify_translation_generator`) accepts nonperiodic input across dirichlet / neumann / open_unknown boundaries, the discovery-bridge mask leakage is closed, and variable-coefficient data generators landed with an empirically-proven admissibility crash test. Nonperiodic support establishes interior differential-operator covariance, **not** boundary-value-problem preservation — the `symmetry_claim` diagnostic carries that distinction. `v0.32.0` / **V0.32** remains the modernization baseline (Python ≥3.12, NumPy 2.x, PySINDy 2.1.x). Legacy Python 3.11 + PySINDy 1.7.5 users track the `release/v0.31.x` maintenance branch (security-only fixes for 12 months from the v0.32.0 tag).

Previous release note (kept for historical context): `v0.30.0` / **V0.30**: nonperiodic-readiness and low-order finite-difference derivative diagnostics. It adds structured boundary-condition metadata (`FieldBatch.SCHEMA_VERSION` bump `0.1` → `0.2`), a `finite_difference` derivative backend for `u_t`, `u_x`, `u_xx` on scalar 1D nonperiodic uniform grids, a `compute_derivatives(backend="auto")` dispatcher, interior-only residual diagnostics for the Heat / Burgers / advection-diffusion / reaction-diffusion strong evaluators, non-blocking ruff / mypy / coverage hygiene, and a narrow declarative release-gate consolidation. KdV, weak evaluators, and translation finite-transform verification remain periodic-only. The V0.29 workflow recipes and support matrix are retained; the V0.28 narrow scalar `xarray.Dataset` path remains part of the stable data surface. No root API expansion; `numpy<2` and Python 3.11-only CI matrix unchanged.
...
```bash
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.38.0rc1"
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.38.0rc1#egg=pdelie[pdebench]"       # optional PDEBench cookbook extra (h5py)
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.38.0rc1#egg=pdelie[downstream]"    # optional PySINDy bridge extra
```

The repository is public and MIT-licensed, so `pip install git+https://…` works without SSH keys, personal access tokens, or a package-index account. Pin a specific tag (e.g. `@v0.37.1`) for reproducibility. TestPyPI and PyPI publication remain deferred to `v1.0` — v0.36f built and hardened the publish path (SHA-pinned actions, build-to-publish hash verification) but deliberately did not exercise it; see [`docs/releases/PUBLISHING.md`](docs/releases/PUBLISHING.md).
