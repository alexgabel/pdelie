...
```bash
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.34.0"
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.34.0#egg=pdelie[pdebench]"       # optional PDEBench cookbook extra (h5py)
python -m pip install "git+https://github.com/alexgabel/pdelie.git@v0.34.0#egg=pdelie[downstream]"    # optional PySINDy bridge extra
```

The repository is public and MIT-licensed, so `pip install git+https://…` works without SSH keys, personal access tokens, or a package-index account. Pin a specific tag (e.g. `@v0.34.0`) for reproducibility. TestPyPI and PyPI publication remain deferred (currently targeted at `v0.36`); see [`docs/releases/PUBLISHING.md`](docs/releases/PUBLISHING.md).
