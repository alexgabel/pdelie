...
The current stable release is `v0.32.0` / **V0.32**: consolidated modernization (Python ≥3.12, NumPy 2.x, PySINDy 2.1.x) plus the v0.32b strict method-score / uncertainty / calibration reporting additive fields, the v0.32c composed candidate-to-discovery workflow example, and the v0.32d external-data readiness cookbooks (one honest PDEBench 1D Burgers slice; a metadata-only The Well feasibility scan concluding `blocked_multichannel_required`). The v0.30.0 line — nonperiodic-readiness and low-order finite-difference derivative diagnostics — is preserved verbatim on the stable data surface. Legacy Python 3.11 + PySINDy 1.7.5 users track the `release/v0.31.x` maintenance branch (security-only fixes for 12 months from the v0.32.0 tag).
