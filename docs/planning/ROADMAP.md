...
| `v0.33` | Nonperiodic generators + mask-preserving discovery bridge | Planning frozen (`feat/v0.33-nonperiodic-generators-planning`) | Wire `BoundaryConditionSpec` into `fit_translation_generator`, `verify_translation_generator`, and `polynomial_translation_svd`; overlap-crop finite-transform verification (rolls in v0.31.5); mask the discovery-bridge design matrix **after** differentiation to close the derivative-stencil leakage. Frozen scope: `docs/design/V0_33_NONPERIODIC_GENERATORS_AND_MASK_PRESERVING_BRIDGE.md`. Sub-milestones `v0.33a` / `v0.33b` / `v0.33c` / `v0.33.0`. No new PDE, no new symmetry method, no `discovery_task_result` schema change (still 22 keys), no root export. |
