...
| `v0.33` | Nonperiodic generators + mask-preserving discovery bridge + variable-coefficient data generator + golden-numbers regression gate | Planning frozen (`feat/v0.33-nonperiodic-generators-planning`) | Wire `BoundaryConditionSpec` into `fit_translation_generator`, `verify_translation_generator`, and `polynomial_translation_svd`; overlap-crop finite-transform verification (rolls in v0.31.5); mask the discovery-bridge design matrix **after** differentiation to close the derivative-stencil leakage; add data-generator support for variable-coefficient profiles (`nu(x)`, `c(x)` on Heat / Burgers / advection-diffusion; the residual-side variable-coefficient support lands in v0.34a); pin frozen per-PDE golden-numbers fixtures into the release-gate to catch silent numerical drift. Frozen scope: `docs/design/V0_33_NONPERIODIC_GENERATORS_AND_MASK_PRESERVING_BRIDGE.md`. Sub-milestones `v0.33a` / `v0.33b` / `v0.33c` / `v0.33d` (parallel scope-widening) / `v0.33e` (parallel hygiene) / `v0.33.0`. No new PDE, no new symmetry method, no `discovery_task_result` schema change (still 22 keys), no root export. |
