...
| `v0.36` | TestPyPI staging + first external symmetry-method port | Planned | TestPyPI trusted-publishing via OIDC on tag push. First external symmetry-method port (Ko-sparse) once the v0.35 point-symmetry registry has proven the multi-method contract with more than one built-in. |
