...
1. **`v0.31.1a` — research spike (Completed via PR #101).** SPEC 0 runtime-compatibility policy + PySINDy 2.x API delta audit + machine-readable compatibility matrix + private compatibility prototype (research-only, not wired into production). Outcome: **A (modern-only future line).**
2. **`v0.30.1` — submodule-only `SymmetryMethod` registry MVP + `SymmetryCandidate` wrapper contract freeze (IN PROGRESS on `feat/v0.30.1-symmetry-method-registry-mvp`).** Ships the representation-neutral `SymmetryCandidate` contract (three implemented representation types + four reserved), the lazy `SymmetryMethod` / `SymmetryMethodResult` / `SymmetryMethodSpec` registry, and one built-in adapter (`polynomial_translation_svd`) wrapping the existing `fit_translation_generator` unchanged. Submodule-only surface — no root `pdelie` export. No external method ports (Ko-style, LieGAN, LaLiGAN, LieGG all deferred). No PyTorch dependency. No package version bump. Per the solo-dev consolidation policy, this code ships publicly as part of `v0.32.0` — no `v0.30.1` tag is created.
3. **`v0.32` — modernization delivery (single big release; solo-dev consolidation policy).** Python ≥3.12 minimum, NumPy 2.x, PySINDy 2.1.x. Internal PR milestones: `v0.32a` (PySINDy 2 migration + numpy floor widening + `setuptools<82` retirement + six API-break sites migrated + `_pysindy2_prototype.py` deleted), `v0.32b` (strict `method_scores` + optional `uncertainty_report` bootstrap over batch/trajectory units + `calibration_report` skeleton), `v0.32c` (composed candidate-to-task workflow example), `v0.32d` (one honest PDEBench readiness cookbook), `v0.32.0` (release close). Only `v0.32.0` gets tagged. CI matrix at close: blocking Python 3.12 / 3.13, advisory Python 3.14. Legacy Python 3.11 lane retires to a `release/v0.31.x` maintenance branch.
