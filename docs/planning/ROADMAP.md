...
| `v0.34` | Variable-coefficient generators + column-normalized weak-form STLSQ | Planned | Extend Heat / Burgers residuals for `ν(x)` / `c(x)`; extend `polynomial_translation_svd` to score constant-coefficient admissibility against variable-coefficient reference fields (the "admissibility crash test"). Column-normalized weak-form STLSQ hardening on the v0.31b2 `WeakPDELibrary` diagnostic. **Correction (v0.34c measurement):** the planning figures of 87× column-scale ratio and condition 111.8 → 3.77 do not reproduce. `pysindy.WeakPDELibrary` seeds its domain centers from the global NumPy RNG with no seed parameter, so the diagnostic was nondeterministic and those figures were one draw from a distribution (canonical fixture measured 5.03–14.44 for the pre-normalization condition number across 12 draws). v0.34c adds an opt-in `seed` kwarg to make the diagnostic reproducible; at the pinned seed the condition-number improvement ranges 1.79×–48.34× across six fixtures (median 4.51×), with the canonical fixture at 1.79×. The gate is a universal 'never worsens conditioning' invariant plus per-fixture pinned values, not a single threshold. Diagnostic-only. No noise-robustness / WSINDy claim. |
