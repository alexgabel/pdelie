...
| `v0.36` | TestPyPI staging + first external symmetry-method port | Planned | TestPyPI trusted-publishing via OIDC on tag push. First external symmetry-method port (Ko-sparse). **Precondition amended in v0.35b:** this previously read "once the v0.35 point-symmetry registry has proven the multi-method contract with more than one built-in". That precondition cannot be satisfied and has been dropped. `SymmetryMethod` requires `fit(field, ...)` — an algorithm that *discovers* a generator from data — whereas a catalogued point symmetry is analytically known and discovers nothing. Registering the catalogue would have meant a `fit()` that ignores its input, and would have made `list_symmetry_methods()` report thirteen methods that never read the data they are handed. The catalogue therefore ships as data in `pdelie.symmetry._point_symmetry_registry`, `SymmetryMethod` still has exactly one built-in, and **the Ko-sparse port is itself what proves the multi-method contract.** v0.36 is unblocked; it is the proof, not the beneficiary of one. |
| `v0.4` | Multi-channel / 2-D contract widening (own release arc) | Planned | Widen `FieldBatch` from `('batch','time','x','var')` to `('batch','time','x','y','var')`. Every residual, derivative, orbit, task-bridge, symmetry method gets a 2-D path. The Well cookbook (currently `blocked_multichannel_required`) conditionally unblocks for genuinely-scalar-2-D shards. Full v0.4a/b/c/d arc scope; not compressible into a single sub-milestone. |
