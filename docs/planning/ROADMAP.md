...
| `v0.34` | Variable-coefficient generators + column-normalized weak-form STLSQ | Planned | Extend Heat / Burgers residuals for `ν(x)` / `c(x)`; extend `polynomial_translation_svd` to score constant-coefficient admissibility against variable-coefficient reference fields (the "admissibility crash test"). Column-normalized weak-form STLSQ hardening on the v0.31b2 `WeakPDELibrary` diagnostic (numbers: 87× column-scale ratio, condition 111.8 → 3.77 after normalization). Diagnostic-only. No noise-robustness / WSINDy claim. |
