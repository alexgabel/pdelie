...
3. **`v0.32a` — modern runtime migration (Completed, merged as PR #103).** Python ≥3.12 minimum, NumPy 2.x, PySINDy 2.1.x. Six API-break sites migrated; `_pysindy2_prototype.py` deleted; `setuptools<82` cap retired; `SymmetryCandidate` reserved-type construction hardened. New CI matrix (Python 3.12/3.13 blocking, 3.14 advisory core-only).
4. **`v0.32b` — strict `method_scores` + optional `uncertainty_report` bootstrap over batch/trajectory units + `calibration_report` skeleton (Completed, merged as PR #104).** Adds three additive fields to `summarize_generator_confidence`, all default-None; migrates the assembled payload to the strict-JSON validation boundary (`_validate_strict_json_compatible`); ships `pdelie.reporting.enrich_method_scores` and `pdelie.symmetry.methods.polynomial_translation_svd.bootstrap_uncertainty` as submodule-only helpers; migrates the built-in method's `method_scores` to the frozen four names `{span_distance, residual_l2, error_curve_max, svd_condition_number}` with direction/description/units metadata; row-level bootstrap refused with `ScopeValidationError` — no silent fallback. No package version bump; no tag; no root exports. Frozen shape in `docs/design/GENERATOR_CONFIDENCE_ADDITIVE_FIELDS.md` and `configs/planning/v0_32_method_scores_scope.json`.
5. **`v0.32c` — composed candidate-to-task workflow example (Completed, merged as PR #105).** New submodule-only composed summary `pdelie.reporting.summarize_candidate_to_discovery_workflow` (`summary_type = "candidate_to_discovery_workflow"`) with 15 explicit stages preserved in fixed order; new JSON-only runnable example `pdelie.examples.candidate_to_discovery_workflow` with a `__main__` CLI and two deterministic scenarios (`"successful"` fully executable; `"valid_but_not_useful_static"` a provenance-backed static illustration). Action policy is caller-configured (never inferred from method scores); heldout is untransformed; split/leakage provenance is emitted for every stage. Controlled example, not a general workflow engine. No package version bump; no tag; no root exports.
6. **`v0.32d` — external-data readiness cookbooks (Completed, merged as PR #106).** One honest PDEBench 1D Burgers scalar-1D slice readiness cookbook (`pdelie.examples.pdebench_burgers_1d_readiness` — pinned to the DaRUS V8 shard `1D_Burgers_Sols_Nu0.001.hdf5`, MD5 `b4be2fc3383f737c76033073e6d2ccfb`, CC-BY-4.0) plus a metadata-only The Well feasibility scan (`pdelie.examples.the_well_feasibility_scan` — paper reports 16 datasets, PolymathicAI catalogue enumerates 23 hosted variants, all `scalar_1d_extractable=False`, conclusion `blocked_multichannel_required`). Two frozen strict-JSON configs under `configs/external_data/`. Optional `h5py` dependency installed via the narrow `[pdebench]` extra (`pip install 'pdelie[pdebench]'`; h5py only, no broad PDEBench support implied). Submodule-only; no broad `from_pdebench` / `from_the_well` adapter; no recovery benchmark claim; no model training; no network I/O in default CI. No package version bump; no tag; no root exports.
7. **`v0.32.0` — release close (Completed on `release/v0.32.0`; awaiting tag review approval).** Version bump 0.31.0 → 0.32.0, `V0_32_RELEASE_READINESS.md`, `support_matrix.v0_32.json`, release-gate manifest consolidated 0.32 row (0.32b/c/d rows collapsed), CI job rename `v0_32-release-gate` → `v0_32_0-release-gate`. Only v0.32.0 is tagged. Legacy Python 3.11 / PySINDy 1.x line retires to a `release/v0.31.x` maintenance branch cut from the `v0.31.0` tag, with a security-only 12-month maintenance-end policy documented in `docs/design/RUNTIME_COMPATIBILITY_POLICY.md`.
...
| `v0.31` | Downstream discovery task bridge | **Completed** — released as `v0.31.0`; this forward row is retained for its scope text, the authoritative record is the completed-milestones table below. | Unlock PySINDy `PDELibrary` config, add `WeakPDELibrary` diagnostic wrapper. Tasks live under `pdelie.tasks.discovery` and `pdelie.tasks.weak_pde_library`, not under the symmetry registry. Design frozen by v0.31a. Runtime lands in v0.31b+. |
| `v0.30.1` | Submodule-only `SymmetryMethod` registry MVP **and** `SymmetryCandidate` wrapper contract freeze | **Completed** — never cut as a standalone tag; the registry and the `SymmetryCandidate` discriminated union shipped inside the v0.31.0 line and are frozen invariants as of v0.35.0. | One built-in adapter (`polynomial_translation_svd`). The adapter returns `SymmetryCandidate`, never `GeneratorFamily` directly. `SymmetryCandidate` is a discriminated-union wrapper representing `GeneratorFamily`, `FormulaGeneratorFamily`, `InvariantMapSpec`, matrix Lie-algebra generators, coordinate vector fields, finite-transform specs, and latent generators — the shape v0.31's discovery bridge, v0.33's Ko infinitesimal-generator adapter, and v0.35a's LieGG extraction all consume without needing further contract rework. No root API. No external method ports. Follows `v0.31` per the discovery-first sequencing agreement. |
| `v0.31.1` | PySINDy 2.x port | **Superseded by v0.32a** — see `docs/design/PYSINDY_2_MIGRATION_AUDIT.md` ("IMPLEMENTED — v0.32a landed the migration"). No `_pysindy_compat.py` shim was ever needed: the modern-only line was chosen and every call site was rewritten directly against pysindy 2.1.x. | Migration release. Introduces `src/pdelie/discovery/_pysindy_compat.py` as a private version-dispatch shim with a `_pysindy_major()` gate plus four helpers (`build_sindy`, `fit_sindy`, `compute_time_derivative`, `build_pde_library`, `build_weak_pde_library`). Absorbs four independent 2.x-API breaks: (i) `SINDy(feature_names=, discrete_time=, t_default=)` constructor kwargs removed — move `feature_names` into `fit()`; (ii) `SINDy.fit(...)` drops `multiple_trajectories/ensemble/library_ensemble/unbias/quiet/replace` and makes `t` positional-required — route ensembling through `EnsemblingOptimizer`; (iii) `SINDy.differentiate` removed — replace `pysindy_model.differentiate(traj, t=t)` in `pdelie.tasks.discovery._compute_residuals_from_predictions` with `model.differentiation_method_(traj, t=t)`; (iv) `PDELibrary`/`WeakPDELibrary` drop `library_functions/function_names/interaction_only` — callers must build a `CustomLibrary`/`PolynomialLibrary` and pass it as `function_library=`. Coordinated `numpy>=2` floor bump (required because `pysindy 2.x` requires `numpy>=2.0` while `pdelie 0.30.0` pins `numpy<2`). Advisory CI job on `pysindy==2.1.0`. No public root export change. |
| `v0.31.5` | Nonperiodic orbit/action scope decision | Rolled into `v0.33b` | Overlap-crop design for nonperiodic translation, previously deferred, now lands as part of `v0.33b`. See `docs/design/V0_33_NONPERIODIC_GENERATORS_AND_MASK_PRESERVING_BRIDGE.md`. |
| `v0.32` | External dataset readiness cookbooks | Completed (tag `v0.32.0` @ `8ec6e4e`, cut 2026-07-27) | One PDEBench 1D Burgers slice readiness (narrow cookbook) and one metadata-only The Well feasibility scan. No recovery benchmark claim. See `docs/releases/V0_32_RELEASE_READINESS.md`. |
| `v0.33` | Nonperiodic **interior-symmetry** and **mask-validity** support + variable-coefficient data generator + golden-numbers regression gate | **Completed** — released as `v0.33.0`; readiness `docs/releases/V0_33_RELEASE_READINESS.md`. All five sub-milestones merged (#110, #111, #113, #114, #115) plus the scope-freeze amendment (#112). Five of six frozen contracts required amendment on contact with measurement; each is recorded in the design freeze. | Wire `BoundaryConditionSpec` into `fit_translation_generator`, `verify_translation_generator`, and `polynomial_translation_svd`; overlap-crop finite-transform verification (rolls in v0.31.5); mask the discovery-bridge design matrix **after** differentiation to close the derivative-stencil leakage, tracked as a three-mask decomposition (observation / derivative-validity / regression-row) with spectral derivatives hard-rejected on masked fields; add data-generator support for variable-coefficient profiles (`nu(x)`, `c(x)` on Heat / Burgers / advection-diffusion; the residual-side variable-coefficient support lands in v0.34a); pin frozen per-PDE golden-numbers fixtures into the release-gate to catch silent numerical drift. **Claim scope:** v0.33a/b establish interior differential-operator covariance on the overlap, **not** boundary-value-problem preservation; the `symmetry_claim` diagnostic carries the distinction and `boundary_value_problem_preserved` is reserved-but-never-emitted in v0.33. Frozen scope: `docs/design/V0_33_NONPERIODIC_GENERATORS_AND_MASK_PRESERVING_BRIDGE.md`. Sub-milestones `v0.33a` / `v0.33b` / `v0.33c` / `v0.33d` (parallel scope-widening) / `v0.33e` (parallel hygiene) / `v0.33.0`. No new PDE, no new symmetry method, no `discovery_task_result` schema change (still 22 keys), no root export. |
| `v0.34` | Variable-coefficient residuals + admissibility scoring + weak-form conditioning | **Completed** — released as `v0.34.0`; readiness `docs/releases/V0_34_RELEASE_READINESS.md`. Three sub-milestones merged: v0.34c (#117), v0.34a (#118), v0.34b (#119). | Heat / Burgers / advection-diffusion residual evaluators accept a pre-sampled `ν(x)` array and dispatch on the `nu_form` the v0.33d generators record (mismatching the form costs ~300× in residual L2). `polynomial_translation_svd.fit()` gains optional reference-relative admissibility scoring in a nested `fit_diagnostics` block — the frozen four score names are unchanged. New `pdelie.symmetry.admissibility.classify_background_treatment` distinguishes a symmetry of a variable-coefficient problem from an equivalence mapping it to a different one (measured separation 77×–15437×, median 1049×). Opt-in column normalization for the weak-form design matrix, plus a `seed` kwarg — **correction:** the planning figures of 87× column-scale ratio and condition 111.8 → 3.77 do not reproduce; `pysindy.WeakPDELibrary` seeds its domain centers from the global NumPy RNG with no seed parameter, so the diagnostic was nondeterministic and those figures were one draw from a distribution. At the pinned seed the improvement ranges 1.79×–48.34× (median 4.51×), canonical 1.79×. Diagnostic-only. No WSINDy / noise-robustness / dataset-recovery claim. |
| `v0.35` | Design diagnostics + symmetry-group registry + optimal-design baselines | **Completed** — released as `v0.35.0`; readiness `docs/releases/V0_35_RELEASE_READINESS.md`. Three sub-milestones merged: v0.35a (#122), v0.35c (#123), v0.35b (#124), plus the day-0 polish PR (#121). Eight of twelve measurement gates changed the shipped contract. **Success criterion 4 is recorded as NOT met:** the point-symmetry catalogue ships as data rather than as registered `SymmetryMethod` adapters, so that contract still has one built-in — see the v0.36 row. | `pdelie.diagnostics.*` first-class utilities (ρ_IR, leverage scores, mutual coherence, restricted eigenvalue); `pdelie.symmetry.point_symmetry_registry` cataloguing known point symmetries per PDE with classification `{exact_row_selection, valid_but_not, invalid}`; optimal-design baselines (leverage-score row selection, QR-pivot, D-optimal design). |
| `v0.36` | Paper-critical migration audit + artifact identity + release hygiene | **Completed** — released as `v0.36.0`; readiness `docs/releases/V0_36_RELEASE_READINESS.md`. Seven sub-milestones merged: day-zero polish (#126), v0.36a-α (#127, #134), v0.36b (#129), v0.36d (#130), v0.36e (#132), v0.36c (#133), v0.36a-β (#135, #136), v0.36f (#137, #138). The migration audit closed at 100 stage comparisons across five PDEs with zero unexplained regressions, verified on macOS and Linux. | **Scope changed during v0.36 day-zero planning.** This row previously read "TestPyPI staging + first external symmetry-method port". The arc was led by the paper-critical pipeline migration audit (`v0.36a`), widened at β from one PDE / 16 stages to five PDEs / 20 stages. Day-zero landed `pdelie.artifact.semantic_hash` as the single canonical hash, the hypothesis->pilot->confirmatory freeze process, the four-class cross-platform portability taxonomy, a mypy fingerprint baseline, and a budgeted portability CI lane. **TestPyPI publication did not happen and is deferred to `v1.0`** — v0.36f built and hardened the publish path without exercising it. **The external symmetry-method port remains at v0.39.** Two forward-scoped design constraints (C-2, C-5) are recorded in `docs/planning/V0_37_BINDING_DESIGN_CONSTRAINTS.md` as `resolves_in_v0_37a`. |
| `v0.37` | Parameter-equivariant actions + admissibility benchmark | **Completed** — released as `v0.37.0`; readiness `docs/releases/V0_37_RELEASE_READINESS.md`. Five sub-milestones merged: day-zero (#140), v0.37a (#141), v0.37b (#142), v0.37c (#143, #144, #145, #146), v0.37d (#147). | Declarative problem-action contracts with five independent relation axes, an exact-shift executor over six runtime paths, a residual commutation report, and a downstream crash test whose four blocking branches are measured — not asserted — to return before discovery runs. The v0.37c two-stage freeze ran **three** pilots and blocked twice, each block catching a specification defect: bounds derived in one norm and measured in another, then a nonperiodic profile on a domain declared periodic. Case C-4 was retired rather than restated. **The point-symmetry registry remains private** — v0.37c's five-case taxonomy is narrower than a public claim would need. TestPyPI publication remains deferred to `v1.0`. |
| `v0.39` | First external symmetry-method port + TestPyPI staging | Planned | Deferred here from v0.36. Ports the Ko infinitesimal-generator method as the second `SymmetryMethod` adapter — **this port is itself what proves the multi-method contract**, which is why v0.35's success criterion 4 is recorded as not met rather than restated: a catalogued point symmetry is analytically known and discovers nothing, so it cannot satisfy a `fit(field, ...)` contract. TestPyPI trusted-publishing via OIDC on tag push follows the port, so the install surface is stressed by a second method before artifacts are handed out. |
| `v0.40` | Multi-channel / 2-D contract widening (own release arc) | Planned | Widen `FieldBatch` from `('batch','time','x','var')` to `('batch','time','x','y','var')`. Every residual, derivative, orbit, task-bridge, symmetry method gets a 2-D path. The Well cookbook (currently `blocked_multichannel_required`) conditionally unblocks for genuinely-scalar-2-D shards. Full v0.4a/b/c/d arc scope; not compressible into a single sub-milestone. |
...
- finite-transform verification on nonperiodic translations (planned for `v0.33b` overlap-crop, superseding the v0.31.5 deferral)
