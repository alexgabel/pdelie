...
| `v0.36` | Paper-critical migration audit + artifact identity + release hygiene | Planned | **Scope changed during v0.36 day-zero planning.** This row previously read "TestPyPI staging + first external symmetry-method port". The arc is now led by the paper-critical pipeline migration audit (`v0.36a-α`): a legacy-vs-modern stage-by-stage comparison across 16 paper-critical stages, with a seven-value migration-label vocabulary and a no-pickle interchange format. Day-zero lands `pdelie.artifact.semantic_hash` as the single canonical hash, the hypothesis→pilot→confirmatory freeze process, the four-class cross-platform portability taxonomy, a mypy fingerprint baseline, and a budgeted portability CI lane. **The external symmetry-method port moves to v0.39** (see that row); TestPyPI staging follows it. Sub-milestone structure `α / b / β / c / d / e / f` is recorded in `docs/planning/PLAN.md`. |
