...
Stable public-surface note for the v0.37.0 release close: the v0.37 arc adds declarative problem-action contracts and an executor (`pdelie.actions`), a five-case admissibility benchmark (`pdelie.benchmarks`), and a downstream action-bundle bridge (`pdelie.downstream`). Every addition is submodule-only and diagnostic-only; `pdelie.__all__` is unchanged. Two new `summary_type` values appear, both on new payloads produced by new functions, and both carry `summary_schema_version` per the measured convention. **`discovery_task_result` keeps its 22-key top level**, frozen since v0.30.1. The point-symmetry registry remains private.
...
v0.37d downstream additions (submodule-only, `pdelie.downstream`): `run_downstream_with_action_bundle` and `classify_branch`. One new `summary_type` -- `pdelie_downstream_task_with_action_bundle` -- emitted by a new function on a new payload. **`discovery_task_result` keeps its 22-key top level**, frozen since v0.30.1: the no-bundle branch returns it unchanged, and the augmented branches emit a different type carrying those 22 keys plus exactly two. `pdelie.__all__` is unchanged.
...
- On the array path the diffusive term follows `parameter_tags["nu_form"]` — `conservative_divergence` or `nonconservative_nu_uxx`. These are different operators for any non-constant `nu`; evaluating the wrong one against matched data inflates the residual L2 by roughly 300×, so the form is dispatched rather than assumed.
- Callable profiles are refused: pass the array the v0.33d generator sampled.
- A coefficient whose kind disagrees with the field's recorded `nu_profile_kind` is **reported** via `coefficient_matches_field_provenance`, never refused — the v0.33d admissibility crash test depends on that combination.
- Conservative *advection* (`c_form = "conservative_divergence"`) is generated by v0.33d but not evaluated in v0.34a; such a field is refused rather than silently evaluated under the wrong operator.
- `ResidualBatch` top-level shape is unchanged; the new keys are additive inside `diagnostics`.
...
- One built-in adapter is registered: `polynomial_translation_svd`. It wraps `fit_translation_generator` without changing numerics; produces one `SymmetryCandidate` with `representation_type = "generator_family"`; deterministic; scalar 1D input; requires a residual evaluator. Periodic input only through v0.32; **since `v0.33a` nonperiodic input is accepted** and dispatches through the interior-only branch. Acceptance is not a claim of boundary-value-problem preservation — the nonperiodic branch establishes interior differential-operator covariance only, reported via the `symmetry_claim` entry in `fit_diagnostics`. The frozen four `method_scores` names are unchanged on both branches.
...
- `SymmetryCandidate` is the single input/output shape that every downstream release consumes: v0.31's discovery task bridge, v0.33's Ko infinitesimal-generator external candidate-generator adapter, and v0.35a's LieGG-style extraction all take and return `SymmetryCandidate`.
...
- v0.30.1 remains submodule-only. No root `pdelie` export. No `pdelie.discover_symmetries`. No external-method port in v0.30.1 (Ko infinitesimal-generator method lands in v0.33 as a distinct experimental adapter).
...
- The composed `TaskResult` schema matches `docs/design/DISCOVERY_TASK_RESULT_SCHEMA.md` exactly. `underlying_discovery_result` embeds `pdelie.discovery.summarize_discovery_result` verbatim, with exactly one documented exception since `v0.33c`: the task attaches its mask diagnostics under the single namespaced key `fit_diagnostics["pdelie_mask_diagnostics"]`. Every other field of the embedded sibling — including every backend-native entry in `fit_diagnostics` — remains byte-for-byte what the backend summarizer produced, and the regression guard strips only that one key before comparing.
